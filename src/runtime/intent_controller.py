    ...
    async def detect(
        ...
        prompt = render_prompt(
            template,
            {
                ...
                "{current_topic_number}": str(current_topic.topic_number),
        ...
        raw_target = validated.target_topic_number
        ...
        if previous_question_strategy == "verify":
            if validated.verification_outcome is None or validated.verification_outcome == "not_applicable":
                if validated.intent == "stop_interview":
                    ...
                    verification_confidence = raw_confidence
                ...
                verification_confidence = validated.verification_confidence
        ...
        if raw_target:
            requested_number = str(raw_target).strip()
            matched = next((t for t in topic_catalog if t.topic_number == requested_number), None)
        ...
        if raw_intent in ("switch_existing_topic", "return_previous_topic") and target_topic_id is None:
            if raw_confidence >= self.confirmation_threshold:
                return IntentDecision(
                    ...
                    raw_explanation=f"User requested topic switch/return but target is absent from the catalog: {raw_target}",
