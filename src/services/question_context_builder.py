...
class QuestionContextBuilder:
    ...
    def _build_target_context(
        ...
        if plan.target_relation_ids:
            ...
            for dep in state.dependencies:
                src_topic = state.find_topic_by_number(dep.source)
                tgt_topic = state.find_topic_by_number(dep.target)
                if src_topic is None or tgt_topic is None:
                    raise ValueError(
                        f"Dependency edge '{dep.source}->{dep.target}' references an unknown topic_number."
                    )
                src_title = src_topic.topic_content
                tgt_title = tgt_topic.topic_content
        ...
        if strat == "confirm_control":
            ...
            if plan.target_topic_id:
                tgt_t = state.find_topic_by_id(plan.target_topic_id)
                if tgt_t:
        ...
        if strat == "fill_gap":
            ...
            if plan.target_slot_ids:
                ...
                target_slot = target_topic.find_slot_by_id(requested_id)
                if not target_slot:
                    raise ValueError(
        ...
        if strat == "resolve_conflict":
            ...
            if plan.target_conflict_slot_ids:
                for sid in plan.target_conflict_slot_ids:
                    s = target_topic.find_slot_by_id(sid)
                    if not s:
                        raise ValueError(
    ...
    def build(
        ...
        target_topic = state.find_topic_by_id(plan.topic_id)
        if not target_topic:
