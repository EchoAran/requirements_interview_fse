...
class EventFactory:
    ...
    def determine_slot_operation(
        ...
        if proposed_op == "conflict":
            return "conflict"
        if proposed_op == "defer_uncertain":
            return "defer_uncertain"
    ...
    def create_slot_value_changed_event(
        ...
        if is_llm_proposed and (effective_new_val is None or str(effective_new_val).strip() == ""):
        ...
        if effective_new_val is None or str(effective_new_val).strip() == "":
            new_state = "empty"
            effective_val = None
        elif op == "defer_uncertain":
            # Deferral with non-empty candidate value
            if slot.state == "filled":
                if slot.value is not None and str(slot.value).strip() == str(effective_new_val).strip():
                    # Pure deferral preserving confirmed value
                    new_state = "filled"
                    effective_val = slot.value
                else:
                    # Deferral introducing a new tentative candidate
                    new_state = "uncertain"
