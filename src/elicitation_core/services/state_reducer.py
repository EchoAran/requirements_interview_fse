...
class StateReducer:
    ...
    def _apply_turn_advanced(cls, state: ProjectState, event: StateEvent, strict_validation: bool = True) -> None:
        ...
        new_turn_idx = int(event.after.get("turn_index", state.turn_index + 1))
        state.turn_index = new_turn_idx
    ...
    def _apply_slot_created(cls, state: ProjectState, event: StateEvent, strict_validation: bool = True) -> None:
        ...
        target_topic = state.find_topic_by_id(topic_id) or state.find_topic_by_number(topic_id)
        if not target_topic:
            if strict_validation:
                raise StateReducerValidationError(f"Topic '{topic_id}' not found for slot_created event {event.event_id}")
            return

        existing = target_topic.find_slot(new_slot.slot_number)
        if not existing:
    ...
    def _apply_slot_value_changed(cls, state: ProjectState, event: StateEvent, strict_validation: bool = True) -> None:
        ...
        for sec in state.sections:
            for top in sec.topics:
                for sl in top.slots:
                    if sl.slot_id == event.entity_id or sl.slot_number == event.entity_id:
                        target_slot = sl
                        break
                if target_slot:
                    break
    ...
    def _apply_topic_status_changed(cls, state: ProjectState, event: StateEvent, strict_validation: bool = True) -> None:
        target_topic = state.find_topic_by_id(event.entity_id) or state.find_topic_by_number(event.entity_id)
        if not target_topic:
            if strict_validation:
