...
class DependencyBuilder:
    ...
    async def build(
        ...
        content_to_num = {d["topic_content"]: d["topic_number"] for d in topics_data}

        def resolve_topic_number(val: object) -> Optional[str]:
            if val is None:
                return None
            s = str(val).strip()
            if s in num_set:
        ...
        for item in raw_edges:
            if not isinstance(item, dict):
                continue
            src = resolve_topic_number(item.get("source"))
            tgt = resolve_topic_number(item.get("target"))

            if src and tgt and src != tgt and src in num_set and tgt in num_set:
                edge_pair = (src, tgt)
