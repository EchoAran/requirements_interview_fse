...
class ProjectPrefiller:
    ...
    async def prefill(
        ...
        if not isinstance(raw_updates, list):
            raise LLMOutputError(f"ProjectPrefiller expected a JSON array, got {type(raw_updates).__name__}")
        ...
        for item in raw_updates:
            if not isinstance(item, dict):
                raise LLMOutputError(f"Each prefill update must be a JSON object, got {type(item).__name__}")
            t_num = str(item.get("topic_number", "")).strip()
            s_num = str(item.get("slot_number", "")).strip()
            s_val_raw = item.get("slot_value")

            if not t_num or t_num not in slots_map:
                raise LLMOutputError(f"Prefill update references unknown topic_number {item.get('topic_number')!r}")
            if not s_num or s_num not in slots_map[t_num]:
                raise LLMOutputError(
                    f"Prefill update references unknown slot_number {item.get('slot_number')!r} in topic '{t_num}'"
                )

            if isinstance(s_val_raw, (list, dict)):
                val = json.dumps(s_val_raw, ensure_ascii=False)
            elif isinstance(s_val_raw, str) and s_val_raw.strip():
                val = s_val_raw.strip()
            else:
                raise LLMOutputError(f"Prefill update for slot '{s_num}' must contain a non-empty 'slot_value'")
