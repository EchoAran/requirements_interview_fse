...
├── scripts/                            # CLI 命令行工具与评估脚本
│   ├── init_project.py                 # 初始化访谈项目
│   ├── step.py                         # 推进对话轮次
│   ├── resume.py                       # 中断恢复与状态审计
│   ├── inspect_state.py                # 检查项目状态快照与槽位完成度
│   ├── replay.py                       # 离线事件流重放 / 离线 LLM 重放
│   ├── benchmark_context_budget.py     # 基于真实运行 Fixture 的上下文预算基准评估
│   └── evaluate_question_generation.py # 专项场景离线生成链路与隔离评测
├── tests/                              # 自动化测试套件
│   ├── fixtures/                       # 标准业务场景静态运行历史数据
│   ├── unit/                           # 各模块单元测试
│   └── integration/                    # 端到端集成测试与全量回归测试
...
系统内置了上下文预算与生成链路的离线自动化评测基准：

```bash
# 1. 运行上下文预算裁剪基准测试 (评估 Prompt 压缩率与关键上下文保留度)
python scripts/benchmark_context_budget.py

# 2. 运行 8 套专项典型业务场景的多轮生成与隔离评测
...
项目配备了严格的自动化测试体系，包含单元测试、8 套工业场景真实 Fixture 回归测试以及断电崩溃恢复测试：
